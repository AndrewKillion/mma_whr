...
class Base:
    ...
    def _setup_fight(
        ...
        fighter_a_obj = self.fighter_by_name(fighter_a)
        fighter_b_obj = self.fighter_by_name(fighter_b)
        fight = Fight(fighter_a_obj, fighter_b_obj, winner, outcome, time_step, handicap, extras)
        return fight
    ...
    def create_fight(
        ...
        fight = self._setup_fight(fighter_a, fighter_b, winner, outcome, time_step, handicap, extras)
        return self._add_fight(fight)

    def _add_fight(self, fight: Fight) -> Fight:
        fight.fighter_a.add_fight(fight)
        fight.fighter_b.add_fight(fight)
    ...
    def load_fights(self, fights: list[str], separator: str = " ") -> None:
        ...
        for line in fights:
            ...
            self.create_fight(fighter_a, fighter_b, winner, int(time_step), handicap, int(outcome), extras)


    def load_fights_from_mma_insights(
        self,
        source: str = "auto",
        limit: int | None = None,
    ) -> int:
